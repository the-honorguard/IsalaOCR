    ...
    *,
    ...
    def api_v2_localization_quality_details():
        ...
        try:
            ...
            return json_error("Ongeldige IoU-drempel.", 400)
        ...
        try:
            ...
            traceback.print_exc()
            return json_error(f"Visuele diagnostiek kon niet worden opgebouwd: {type(exc).__name__}: {exc}", 500)
    ...
    def api_v2_localization_split():
        ...
        try:
            if action == "auto":
                ...
                return json_error("Onbekende splitactie", 400)
    ...
    def api_v2_create_job():
        ...
        if action_id not in actions:
            return json_error("Onbekende taak", 400)
        if action_id in {"24", "25", "26", "27", "28"} and not current_recognition_gate().get("ready"):
            gate = current_recognition_gate()
            return json_error(f"Recognition is vergrendeld: {gate.get('reason') or 'goedgekeurde Recognition-GT ontbreekt'}.", 423)
        ...
        if existing is not None:
            return json_error("Deze taak draait al.", 409, job=existing)
        ...
        if action_id == "2" and options.get("table_model_id") is not None:
            ...
            if table_model_id and not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._:-]{0,255}", table_model_id):
                return json_error("Ongeldig table-cell model-ID", 400)
        ...
        if action_id == "26":
            ...
            if device not in {"cpu", "gpu"}:
                return json_error("Ongeldig device", 400)
