...
def ambiguous_lateral_suffixes(fields: Iterable[dict[str, Any]]) -> set[str]:
    """Return measurement suffixes for which both LV and RV targets exist."""
...
def relation_lateral_side(relation: dict[str, Any]) -> str:
    """Read only explicit LV/RV evidence; never infer a side from row order."""
    parts = [
        relation.get("label_text"),
        relation.get("context_text"),
        relation.get("panel_name"),
        relation.get("panel_label"),
        relation.get("table_name"),
        relation.get("table_label"),
        relation.get("column_header"),
        relation.get("header_text"),
    ]
...
def lateral_candidate_allowed(
    ...
    """Prevent arbitrary LV/RV guesses when a generic label fits both fields.

    If only one lateral target exists for a measurement, the historical generic
    alias behaviour remains valid. If both LV and RV counterparts exist, an
    automatic mapping requires explicit matching lateral evidence in the label,
    context, panel or table metadata. A generic label such as ``Ejection
    Fraction`` or ``Stroke Volume`` therefore remains open for review instead of
    being assigned to whichever side happens to win a score tie.
    """
    suffix = field_lateral_suffix(field)
    side = field_lateral_side(field)
