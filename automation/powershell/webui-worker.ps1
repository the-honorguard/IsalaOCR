...
foreach($name in @("pending","running","completed","failed","status","logs")){New-Item -ItemType Directory -Force -Path (Join-Path $JobsRoot $name)|Out-Null}
...
function Get-LiveProgressLabel {
    ...
    try {
        # Docker/Compose uses both stdout and stderr. Inspect the most recently
        # changed stream first, while keeping the files separate for the UI.
...
function Set-WorkerState {
    ...
    $payload=@{
        ...
        started_at=$script:WorkerStartedAt
...
function Complete-InterruptedJobs {
    ...
    foreach($runningJob in @(Get-ChildItem $runningRoot -Filter "*.json" -File -ErrorAction SilentlyContinue)){
        try {
            ...
            $statusFile=Join-Path (Join-Path $JobsRoot "status") ($jobId+".json")
            Write-JsonUtf8NoBom -Value $data -Path $statusFile -Depth 6
            $workerLogFile=Join-Path (Join-Path $JobsRoot "logs") ($jobId+".worker.log")
            Add-WorkerLog -Path $workerLogFile -Message "Deze taak was nog actief toen de worker opnieuw werd gestart. De taak is veilig als mislukt gemarkeerd; voer hem opnieuw uit."
            ...
            Write-Warning "Kon onderbroken taak $($runningJob.Name) niet herstellen: $($_.Exception.Message)"
        }
    }
}
...
if(Test-Path $lock){
    try{
        ...
        if(Test-IsalaWorkerState $old -RequireCurrentVersion){
            Write-Host "Een gezonde IsalaOCR webworker draait al (PID $($old.pid))." -ForegroundColor Yellow
            exit 0
        }
        if(Test-IsalaWorkerState $old){
            Write-Host "Een verouderde IsalaOCR-worker wordt gestopt (PID $($old.pid), versie $($old.worker_version))." -ForegroundColor Yellow
            Stop-IsalaProcessTree -ProcessId ([int]$old.pid)
            Start-Sleep -Milliseconds 500
        } else {
            Write-Host "Verouderde workerstatus verwijderd; de geregistreerde worker is niet meer gezond." -ForegroundColor Yellow
        }
    }catch{
        Write-Host "Ongeldige workerstatus verwijderd." -ForegroundColor Yellow
    }
...
try {
    & (Join-Path $PSScriptRoot "preparation-status.ps1")
} catch {
    Write-Warning ("Kon voorbereidingsstatus bij workerstart niet controleren: {0}" -f $_.Exception.Message)
}
    ...
    while($true){
        ...
        try{
            ...
            if([string]$data.job_type -eq "artifact_delete"){
                ...
                if([string]$data.action_id -in @("20","21","22")){
                    $safeSource=([string]$data.options.source_id).Replace('"','""')
                    if(-not [string]::IsNullOrWhiteSpace($safeSource)){
                        $powerShellCommand += ' "{0}"' -f $safeSource
                    }
                }
                elseif([string]$data.action_id -eq "2"){
                    $safeTableModel=([string]$data.options.table_model_id).Replace('"','""')
                    if(-not [string]::IsNullOrWhiteSpace($safeTableModel)){
                        $powerShellCommand += ' "{0}"' -f $safeTableModel
                    }
                }
                elseif([string]$data.action_id -eq "26"){
                    $safeDevice=([string]$data.options.device).Replace('"','""')
                    if(-not [string]::IsNullOrWhiteSpace($safeDevice)){
                        $powerShellCommand += ' "{0}"' -f $safeDevice
                    }
                }
            }
            ...
            $wrapperLines=@(
                '@echo off',
                'setlocal',
                ('set "ISALA_PROJECT_ID={0}"' -f $jobProjectId),
                $powerShellCommand,
                'set "ISALA_EXIT=%ERRORLEVEL%"',
                ('>"{0}" echo %ISALA_EXIT%' -f $exitCodeFile.Replace('"','""')),
                'exit /b %ISALA_EXIT%'
            )
            $wrapperEncoding=New-Object System.Text.ASCIIEncoding
            [System.IO.File]::WriteAllLines($wrapperFile,$wrapperLines,$wrapperEncoding)
            ...
            if(Test-Path -LiteralPath $exitCodeFile -PathType Leaf){
                $rawExit=([string](Get-Content -LiteralPath $exitCodeFile -Raw -ErrorAction SilentlyContinue)).Trim()
                $capturedExit=0
                if([int]::TryParse($rawExit,[ref]$capturedExit)){
                    $exit=$capturedExit
                }
            }
            # Defensive fallback for Windows PowerShell/Start-Process edge cases:
            # a launcher-level fatal error must never be presented as success.
            if($exit -eq 0){
                $combinedOutput=''
                if(Test-Path -LiteralPath $logFile){$combinedOutput += [string](Get-Content -LiteralPath $logFile -Raw -ErrorAction SilentlyContinue)}
                if(Test-Path -LiteralPath $errorFile){$combinedOutput += "`n" + [string](Get-Content -LiteralPath $errorFile -Raw -ErrorAction SilentlyContinue)}
                if($combinedOutput -match '(?m)^IsalaOCR failed:\s*$' -or $combinedOutput -match '(?m)^Action failed:\s*$'){
                    $exit=1
                    Add-WorkerLog -Path $workerLogFile -Message "Fatale launcherfout gevonden in scriptuitvoer; taakstatus gecorrigeerd naar mislukt."
                }
            }
        }catch{
            ...
            Remove-Item $wrapperFile,$exitCodeFile -Force -ErrorAction SilentlyContinue
        }
        ...
        if($exit -eq 0){Write-Host "[$(Get-Date -Format HH:mm:ss)] Voltooid: $($data.job_id)" -ForegroundColor Green}else{Write-Host "[$(Get-Date -Format HH:mm:ss)] Mislukt: $($data.job_id) (exit $exit)" -ForegroundColor Red}
        Set-WorkerState -State "idle"
    }
}finally{
    Remove-Item $lock -Force -ErrorAction SilentlyContinue
