    ...
    while($true){
        ...
        try{
            ...
            if([string]$data.job_type -eq "artifact_delete"){
                ...
                elseif([string]$data.action_id -in @("20","21","22","58","62")){$safeSource=([string]$data.options.source_id).Replace('"','""');if(-not [string]::IsNullOrWhiteSpace($safeSource)){$powerShellCommand += ' "{0}"' -f $safeSource};if([string]$data.action_id -eq "62"){$safeTableModel=([string]$data.options.table_model_id).Replace('"','""');if(-not [string]::IsNullOrWhiteSpace($safeTableModel)){$powerShellCommand += ' -TableModelId "{0}"' -f $safeTableModel}}
                elseif([string]$data.action_id -in @("2","59")){$safeTableModel=([string]$data.options.table_model_id).Replace('"','""');$safeSource=([string]$data.options.source_id).Replace('"','""');if([string]$data.action_id -eq "59"){if(-not [string]::IsNullOrWhiteSpace($safeSource)){$powerShellCommand += ' "__regions_only_source__:{0}"' -f $safeSource}else{$powerShellCommand += ' "__regions_only__"'}}elseif([bool]$data.options.render_only){$powerShellCommand += ' "__render_only__"'}elseif(-not [string]::IsNullOrWhiteSpace($safeSource)){$powerShellCommand += ' "__source_only__:{0}"' -f $safeSource}elseif(-not [string]::IsNullOrWhiteSpace($safeTableModel)){$powerShellCommand += ' "{0}"' -f $safeTableModel}}
